...
Window::Window(HINSTANCE hInstance, std::function<LRESULT(HWND, UINT, WPARAM, LPARAM)> win_proc, int client_width, int client_height, const std::string& title) :
	...
	m_ex_style(0),
	m_screen_transition(ScreenTransitionDetails{ 0, 0, 0, 0 }),
	m_custom_win_proc(win_proc)
{
	m_id = get_next_id();
...
Window::~Window()
{
	...
	UnregisterClass(std::filesystem::path(m_id).wstring().c_str(), m_hInstance);
}
...
{
	const std::wstring classID = std::filesystem::path(m_id).wstring();
...
{
	const std::wstring class_id = std::filesystem::path(m_id).wstring();
	const std::wstring title = std::filesystem::path(m_title).wstring();
...
void Window::set_fullscreen(bool fullscreenState)
{
	...
	int& prev_x = m_screen_transition.prev_x;
	int& prev_y = m_screen_transition.prev_y;
	int& prev_width = m_screen_transition.prev_width;
	int& prev_height = m_screen_transition.prev_height;
...
LRESULT Window::handle_proc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	return m_custom_win_proc(m_hwnd, uMsg, wParam, lParam);
}
